...
KNOWN_NAMES = [ ... ]  # Replace with your actual list of known names
...
def clean_entity(name):
    ...
    """
    name = re.sub(r'[^A-Z\s\-]s', '', name.upper())
...
def categorize_transaction(row):
    """
    Categorizes a transaction based on its description and debit/credit values.
    
    Args:
        row (pd.Series): A row from the DataFrame containing transaction details.
    
    Returns:
        str: The transaction type ('VAT', 'Bank Charges', 'In', 'Out', or 'Unknown').
    """
...
def highlight_and_save_excel(df, output_path):
    """
    Saves a DataFrame to Excel and highlights rows where Transaction Type is 'Bank Charges'.
    ...
    wb.save(output_path)
    logging.info(f"Saved Excel file with highlights: {output_path}")

def export_summary(df):
    """
    Creates a summary of transactions grouped by Extracted Name and Transaction Type.
    
    Args:
        df (pd.DataFrame): The DataFrame containing transaction data.
    
    Returns:
        pd.DataFrame: A pivot table summarizing transaction counts.
    """
    ...
    for uploaded_file in uploaded_files:
        ...
        if 'description' in df.columns:
            ...
            for i in range(total_rows):
                df.iloc[i, df.columns.get_loc('Extracted Name')] = extract_transaction_name(df.iloc[i][df.columns.get_loc('description')])
